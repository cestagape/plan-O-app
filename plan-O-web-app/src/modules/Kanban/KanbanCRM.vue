<template>
  <BCardGroup deck>
    <BCard
      ...
      class="px-0 m-0 border-0 shadow-lg"
        ...
        <template #item="{ element, index }">
          ...
          >
            ...
            <BCardBody class="p-1 fw-light">
              ...
              <p>
                Сумма <br /><span class="fw-semibold">{{ element.price }}</span>
              </p>
...
export default {
  ...
  methods: {
    ...
    handleDragEnd(event) {
      ...
      const draggedItem = this.lists[from.id][oldIndex];

      // Update the status of the dragged item to match the new column
      draggedItem.status = to.id;

      // Optionally, you can update the order of items in the list as well
      this.lists[from.id].splice(oldIndex, 1);
      this.lists[to.id].splice(newIndex, 0, draggedItem);
